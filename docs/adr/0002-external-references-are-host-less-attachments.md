...
An external reference is deliberately *not* an Attachment for field purposes: null `host_type`/`host_id`/`field_name`, excluded from every field-context query, so `HasMedia` never returns one and it can never affect what a host model renders. It participates only in the usage list and the usage count (and therefore the picker's in-use/unattached facet). Creation is code-only, since it needs the application's identifier and label; revocation is exposed in the management page's usage panel, so a stale reference whose creating code is gone cannot block deletion forever.

Identity is the pair (asset, identifier), held by a unique index rather than by an application read-then-write, so registering the same reference twice is idempotent however two runs interleave, and a rerun that names only the identifier leaves the existing label standing. The guard that keeps a host model's row out of a revoke sits on the write itself rather than on what the usage panel renders, so "host rows are not removable" holds by construction: detaching stays the host record's own act whatever a caller hands the revoke.
